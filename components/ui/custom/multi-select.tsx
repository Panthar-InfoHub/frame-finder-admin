...
export function MultiSelectContent({
  ...
  // Only count MultiSelectItem children
  const multiSelectItems = React.Children.toArray(children).filter(
    (child: any) => child?.type?.name === "MultiSelectItem"
  );

  const hasItems = multiSelectItems.length > 0;
      ...
      <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-0">
        <Command {...props}>
          ...
          <CommandList>
            ...
            {!hasItems ? <CommandEmpty>{emptyMessage}</CommandEmpty> : null}
            {children}
          </CommandList>
        </Command>
      </PopoverContent>
